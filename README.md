...
Add items by pasting a product URL. Money Penny scrapes the product name, price, images, and variants automatically. A background scheduler re-checks prices on a configurable interval and notifies you via WhatsApp when prices drop or rise.
...
Money Penny can send you price alerts and accept commands over WhatsApp using Twilio's free sandbox. This is entirely optional — all features work without it.
...
For Money Penny to receive WhatsApp messages (so you can send URLs and commands), Twilio needs to reach your local server.
...
In the Money Penny web UI, go to **Settings** and toggle on:
...
Some sites (e.g. Urban Outfitters) return 403 errors or render product data only via JavaScript. Money Penny handles this in two ways:
